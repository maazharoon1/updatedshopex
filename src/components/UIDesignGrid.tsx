...
function UIDesignCard({ project }: { project: Project }) {
  ...
  return (
    <a href={project.liveUrl} target="_blank" rel="noopener noreferrer" className="ui-design-card group block min-w-0 text-left" aria-label={`Visit ${project.title} website (opens in a new tab)`}>
      <span ref={windowRef} className="ui-design-window" style={{ "--preview-offset": `${-overflow}px` } as CSSProperties}>
        ...
        />
        {failed && <span className="absolute inset-0 grid place-items-center p-4 text-sm text-white">Preview unavailable. Visit website.</span>}
      </span>
    </a>
  );
}
...
export function UIDesignGrid({ projects }: { projects: Project[] }) {
  return <div className="container-wide grid grid-cols-1 gap-5 sm:grid-cols-2 sm:gap-6 xl:grid-cols-4">
    {projects.map((project) => <UIDesignCard key={project.id} project={project} />)}
  </div>;
}
